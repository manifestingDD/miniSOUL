...
from library.optimization import compute_target_reward, minimizer
...
def show_minimization():
    ...
    # 3) Clear maximizer results if they exist when entering maximization page
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    with tab1:
        ...
        if st.session_state['minimizer_region_validated'] == "Not Validated" and region_code_minimizer:
            ...
            # 4) User select {target attendance, target period} 
            # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            col1, col2, col3 = st.columns([2, 1, 7])
            with col1:
                reward_goal = st.number_input("", value = None, step = 1, placeholder= f"Target {target_var}...")
            with col3:
                st.write("")
            ...
            message = f"Total {target_var} with current plan = {current_gain}"
            st.markdown(
            ...
            if st.button("Let's begin!"):
                with st.spinner("I'm working on it ..."):
                    final_spend_plan, allocation_rank, success = minimizer(
                        reward_goal = reward_goal,
                        media_list = media_list,
                        base_spend_plan = df_base,
                        spend_plan = spend_plan,
                        future_spend_plan= df_future, 
                        df_base = df_base,
                        df_time = df_time,
                    ...
                    if success == 0:
                        st.error(f"Optimization failed: failed to reach the {target_var} target after all medias reaching spending upper bounds. Please lower the {target_var} goal or increase budget.")
                    if success == 1:
                        ...
                        st.session_state['minimizer_artifacts'] = optimization_artifacts
    ...
    with tab2:
        ...
        if scenario_status:
            artifacts = st.session_state['minimizer_artifacts']
            st.write("")
            st.write("")
            ...
            if viewing == 'Aggregate Summary':
                ...
                if spend_change <= 0: 
                    verbal_summary1 = f"Now this is nice. With the optimized spend, we achieve the {target_var} goal with {color_spend_change} less total Ad budget compared to the original plan. That's {color_roas_change} up in return on ad spend!"
                else:
                    verbal_summary1 = f"We have a pretty high target but we can still achieve it as we go close to the upperbounds of media channels. Overall, we can achieve the {target_var} goal at a cost of {color_spend_change} more total Ad budget." 
